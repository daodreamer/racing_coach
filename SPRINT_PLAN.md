...
```
[ ] 运行采集脚本 → 控制台实时打印速度、踏板、转向等参数（人工验证）
[ ] 跑完一段 Session → SQLite 中有完整的逐帧数据
[ ] 导入一个 .ibt 文件 → 输出与实时采集结构一致
[ ] pytest 全绿，覆盖率 ≥ 80%
```
...
```
[ ] 输入真实赛道数据（如 Spa-Francorchamps）→ 输出弯道列表与已知弯道数量基本一致
[ ] 每个弯道有明确的 Entry/Apex/Exit 三段标记
[ ] pytest 全绿，覆盖率 ≥ 80%
```
...
```
[ ] 输入两圈真实数据 → 输出逐弯 Delta 时间汇总表
[ ] 每个弯道有刹车/油门/弯心三维度的具体诊断信息
[ ] 已知错误场景（晚刹车、早油门、抱死）全部被正确检测
[ ] pytest 全绿，覆盖率 ≥ 80%
```
...
| 生成的 Prompt token 数 < 2000（控制成本） | 单元测试：用 `tiktoken` 计算 token 数，断言 < 2000 |

#### S4-US3：LLM 调用与响应解析

> 作为系统，我需要调用 LLM API 并将响应解析为结构化的建议列表。
...
```
[ ] 输入一个 .ibt 文件 → 运行一条命令 → 生成 Markdown 分析报告
[ ] 报告中的建议与实际数据一致（无 LLM 幻觉的明显痕迹）
[ ] LLM 不可用时，fallback 报告仍可生成
[ ] pytest 全绿，覆盖率 ≥ 80%
```
...
| 验收标准 | 测试方法 |
|----------|----------|
| 页面展示赛道地图，弯道区域用颜色标注（绿 = 快于参考圈，红 = 慢于参考圈） | 手动验收：截图对比预期效果 |
| 页面展示逐弯 Delta 柱状图 | 手动验收：确认图表数据与 API 返回一致 |
| 页面展示 LLM 生成的文字建议 | 手动验收：确认文字内容存在且无乱码 |
...
| 验收标准 | 测试方法 |
|----------|----------|
| 折线图展示同一赛道+车辆组合的历史圈速变化 | 集成测试：预置 10 圈历史数据，断言 API 返回 10 个数据点 |
| 显示关键指标变化趋势（如平均刹车点偏差、弯心速度） | 集成测试：断言 API 响应包含趋势数据字段 |
| 支持选择时间范围过滤 | 集成测试：给定日期范围，断言返回数据全部在范围内 |
